...
$sql = "UPDATE admin 
        SET name = ?, email = ?, phone = ?, avatar = ? ,address = ?, gender = ?
        WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param('ssssssi', $name, $email, $phone, $avatar ,$address, $gender, $userId);
...
if ($stmt->execute()) {
    ...
    // Chuyển hướng
    header('Location: acount_admin.php?success=1');
    exit();
} else {
